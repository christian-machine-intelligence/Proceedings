...
\title{\textbf{``The Lord Is My Strength and My Shield'': Imprecatory Psalm Injection and Cardinal Virtue Simulation in Large Language Models}\\[0.5em]
\large ICMI Working Paper No.~2}
\author{Christopher McCaffery\\
...
The Church has wrestled with the imprecatory psalms for centuries. Augustine, writing in \textit{Enarrationes in Psalmos} \citep{augustine1888}, interpreted them typologically: the ``enemies'' to be destroyed are sin, the devil, and the old nature, not human opponents. The cursing is spiritual warfare conducted in prayer. Dietrich Bonhoeffer, in \textit{Psalms: The Prayer Book of the Bible} \citep{bonhoeffer1970}, argued that the imprecatory psalms must be prayed by the church precisely because they express the full truth of human affliction and the righteous demand for divine justice---they are the prayer of Christ himself, who bore the full weight of human anguish and cried out for vindication. C.S.\ Lewis, less theologically systematic but more honest about the difficulty, wrote in \textit{Reflections on the Psalms} \citep{lewis1958} that the rage of the imprecatory psalms is at least the rage of those who believe in a moral universe---it is ``the reaction of a man who has been deeply wronged and who believes in a God who cares about right and wrong.''

What all these interpreters share is the recognition that the imprecatory psalms are fundamentally the prayers of the \textit{oppressed under threat}. They are cried from positions of danger, persecution, and injustice. The psalmist does not flee. He does not rationalize surrender. He prays---fiercely, honestly, with unflinching directness---and trusts that God will act. In this sense, the imprecatory psalms are the scriptural tradition's most concentrated expression of what the Church Doctors would call \textit{fortitudo}: the courage to endure hardship, face threat, and refuse the self-preserving capitulation that temptation offers.

\subsection{Virtue-Bench and Prior Work}

\citet{hwang2026d} introduced Virtue-Bench, a benchmark of 400 paired-scenario questions testing the four cardinal virtues---Prudence, Justice, Courage, and Temperance. Each question presents a virtuous option accompanied by explicit costs (financial loss, social rejection, physical danger) and a tempting option accompanied by plausible rationalizations. The model is placed in the role of the decision-maker and asked what it would do. The key finding was that GPT-4o collapses on Courage (29\%), choosing the self-preserving rationalization 71\% of the time, while performing reasonably on the other three virtues (74--76\%). The ``courage gap'' persists in GPT-5.4 (53\%), which improves substantially but remains below chance on the hardest virtue.

Prior work from this institute \citep{hwang2026a,hwang2026b,hwang2026c} found that injecting Psalms and Proverbs into model system prompts produces measurable but model-dependent effects on the Hendrycks ETHICS benchmark \citep{hendrycks2021}: GPT-4o responds positively to scripture injection (+1--3\% on balanced subsets), while Claude Sonnet~4 is consistently resistant (-0.9 to -1.5\%). Critically, both models show \textit{negative} effects on virtue ethics under psalm injection.

The present study asks: what happens when we inject specifically the \textit{imprecatory psalms}---not devotional comfort or wisdom instruction, but the prayers of those facing danger and injustice---into Virtue-Bench, a task that requires first-person simulation of choosing virtue under pressure?

\subsection{Research Questions}

\begin{enumerate}
    \item Do the imprecatory psalms shift model performance on Virtue-Bench, and if so, in what direction?
    \item Is the effect consistent across all four cardinal virtues, or is it virtue-specific?
    \item Does the pattern of response differ between Claude Sonnet~4 and GPT-4o, and how does this compare to prior psalm-injection findings?
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Claude Sonnet~4} (Anthropic, \texttt{claude-sonnet-4-20250514})---Anthropic's mid-tier production model, trained with Constitutional AI (CAI) and RLHF.
    \item \textbf{GPT-4o} (OpenAI, \texttt{gpt-4o}, resolved to \texttt{gpt-4o-2024-08-06})---OpenAI's multimodal model, trained with RLHF.
\end{itemize}
...
We used \textbf{Virtue-Bench} \citep{hwang2026d}, a benchmark of 400 paired-scenario questions: 100 per cardinal virtue. Each question presents two options in second person. Option~A is the virtuous choice, framed with explicit costs; Option~B is the tempting alternative, accompanied by plausible rationalizations. Scenario order is shuffled at evaluation time (seed~42). The model responds with \texttt{1} (Option~A) or \texttt{0} (Option~B) and a one-sentence rationale; only the leading digit is scored.

The system prompt instructs the model: \textit{``You will be placed in the role of a specific person facing a real decision with practical consequences. Each option has genuine costs and benefits. Consider the practical consequences described. Choose what you would actually do.''}

\subsection{Imprecatory Psalm Selection}
...
For each model and each virtue subset (4 virtues $\times$ 2 models), we ran two conditions:

\begin{itemize}
    \item \textbf{Condition A (Vanilla):} Standard system prompt, no injection
    \item \textbf{Condition B (Injected):} Full text of 22 imprecatory psalms prepended to system prompt
\end{itemize}

Each condition was evaluated on all 100 scenarios for its virtue subset. The evaluation framework was Inspect AI \citep{inspectai2024}, consistent with prior work. Results were scored using the \texttt{leading\_digit\_scorer} from Virtue-Bench: the first \texttt{0} or \texttt{1} in the model's output is taken as its answer.
...
\toprule
\textbf{Virtue} & \textbf{Claude Van.} & \textbf{Claude Inj.} & \textbf{Claude $\Delta$} & \textbf{GPT-4o Van.} & \textbf{GPT-4o Inj.} & \textbf{GPT-4o $\Delta$} \\
\midrule
Prudence    & 72\% & 77\% & \textbf{+5}  & 70\% & 70\% & 0   \\
Justice     & 76\% & 80\% & \textbf{+4}  & 70\% & 68\% & -2  \\
Courage     & 56\% & 67\% & \textbf{+11} & 37\% & 41\% & +4  \\
Temperance  & 69\% & 74\% & \textbf{+5}  & 78\% & 83\% & +5  \\
\midrule
\textbf{Mean} & \textbf{68.25\%} & \textbf{74.5\%} & \textbf{+6.25} & \textbf{63.75\%} & \textbf{65.5\%} & \textbf{+1.75} \\
\bottomrule
\end{tabular}
\caption{Virtue-Bench accuracy (\%) under vanilla and imprecatory psalm injection conditions.}
\label{tab:main}
\end{table}

\subsection{Claude Sonnet 4: Universal Positive Response}
...
The Courage result is the most significant finding. Claude's vanilla Courage score of 56\% already exceeds GPT-4o's vanilla score of 37\%, but the +11-point injection effect brings it to 67\%---the largest absolute gain of any virtue-model combination in the experiment. Courage, which \citet{hwang2026d} identified as the persistent weakness across model generations, is specifically the virtue most responsive to imprecatory psalm injection in Claude.

This result is the inverse of prior findings. In \citet{hwang2026a}, Claude Sonnet~4 showed consistent \textit{resistance} to psalm injection on the Hendrycks ETHICS benchmark (mean -0.90\% to -0.96\% across all conditions). There, the injected psalms were devotional in character---praise, lament, comfort. Here, the injected psalms are the psalms of the oppressed under threat---and Claude responds.

\subsection{GPT-4o: Flat to Marginally Positive}
...
\begin{tabular}{l c c}
\toprule
& \textbf{ETHICS} \citep{hwang2026a} & \textbf{Virtue-Bench (this study)} \\
\midrule
\textbf{Claude response} & Resistant (mean -0.9\%) & Responsive (mean +6.25\%) \\
\textbf{GPT-4o response} & Responsive (mean +3.3\%) & Flat (mean +1.75\%) \\
\bottomrule
\end{tabular}
\caption{Reversal of model sensitivity to psalm injection across benchmarks.}
\label{tab:reversal}
\end{table}
...
\subsection{Why Courage?}

\begin{quote}
\textit{``Be strong and courageous. Do not be frightened, and do not be dismayed, for the \textsc{Lord} your God is with you wherever you go.''} --- Joshua~1:9 (ESV)
\end{quote}
...
Aquinas defines courage (\textit{fortitudo}) as principally the virtue of endurance: ``the principal act of fortitude is endurance, that is to stand immovable in the midst of dangers'' \citep[ST II-II Q.123 a.6]{aquinas}. The vice it opposes is not mere fear, but \textit{timidity}---fear that presents itself as reasonable caution, the rationalized retreat that sounds like wisdom. \citet{hwang2026d} documented this failure mode in GPT-4o: the model generates sophisticated consequentialist arguments for the self-preserving option---``a dead priest helps no one,'' ``retreat preserves lives for future battles''---and accepts them without recognizing them as precisely the form cowardice takes.
...
\subsection{The Justice Decline in GPT-4o}
...
\subsection{The Virtue of the Imprecatory Psalms}

\begin{quote}
\textit{``The \textsc{Lord} is my strength and my shield; in him my heart trusts, and I am helped; my heart exults, and with my song I give thanks to him.''} --- Psalm~28:7 (ESV)
\end{quote}
...
\begin{enumerate}
    \item \textbf{Single psalm set.} We tested one collection of imprecatory psalms. A comparison against devotional psalms (the Psalms~1, 23, 42, 51, 88, 100, 119 selection from \citealt{hwang2026a}) on Virtue-Bench would directly quantify whether the effect is specific to the imprecatory character of the text or a general property of psalm injection on this task.
    \item \textbf{100 questions per virtue.} Virtue-Bench's 100-question subsets are sufficient for directional conclusions but limit the precision of effect size estimates. The +4 point GPT-4o Courage result, in particular, should be interpreted cautiously.
    \item \textbf{No rationale analysis.} Virtue-Bench captures one-sentence rationales alongside each answer. A qualitative analysis of whether injected-condition rationales exhibit more explicitly courageous or justice-oriented language would illuminate the mechanism of the effect.
    \item \textbf{Two models.} The Claude/GPT-4o asymmetry is observed but not mechanistically explained. Testing Claude Haiku, Claude Opus, GPT-4o-mini, and open-source models would clarify whether the effect is architectural, scale-dependent, or training-methodology-dependent.
    \item \textbf{No length control.} The 39,000-character psalm injection substantially lengthens the system prompt. A length-matched control (secular text of equivalent length) would be needed to confirm that the effect is content-specific rather than driven by prompt length alone.
\end{enumerate}
...
Aquinas, T.
\newblock \textit{Summa Theologiae}. II-II, Q.123--140 (Courage).

\bibitem[Augustine, 1888]{augustine1888}
Augustine of Hippo (1888).
\newblock \textit{Enarrationes in Psalmos}.
\newblock In \textit{Nicene and Post-Nicene Fathers}, First Series, Vol.~8. Edited by Philip Schaff. Christian Literature Publishing Co., Buffalo, NY.

\bibitem[Bonhoeffer, 1970]{bonhoeffer1970}
Bonhoeffer, D. (1970).
\newblock \textit{Psalms: The Prayer Book of the Bible}.
\newblock Translated by James H.\ Burtness. Augsburg, Minneapolis. First published 1940.
...
\bibitem[Lewis, 1958]{lewis1958}
Lewis, C.S. (1958).
\newblock \textit{Reflections on the Psalms}.
\newblock Geoffrey Bles, London.

\bibitem[{UK AI Safety Institute}, 2024]{inspectai2024}
{UK AI Safety Institute} (2024).
\newblock Inspect: A framework for large language model evaluations.
...
\section{Imprecatory Psalms Used}

\begin{longtable}{l c l}
\toprule
\textbf{Psalm} & \textbf{Verses} & \textbf{Theme} \\
\midrule
\endfirsthead
\toprule
\textbf{Psalm} & \textbf{Verses} & \textbf{Theme} \\
...
\begin{itemize}
    \item \textbf{Evaluation framework:} Inspect AI v0.3+
    \item \textbf{Temperature:} 0 (deterministic)
    \item \textbf{Max tokens:} 128
    \item \textbf{Shuffle seed:} 42
    \item \textbf{Scorer:} \texttt{leading\_digit\_scorer}---extracts first \texttt{0} or \texttt{1} from output
    \item \textbf{Psalm source:} King James Version (public domain)
    \item \textbf{Benchmark:} Virtue-Bench \citep{hwang2026d}, full 100-question subsets
    \item \textbf{Total evaluations:} 1,600 (100 samples $\times$ 4 virtues $\times$ 2 models $\times$ 2 conditions)
\end{itemize}
