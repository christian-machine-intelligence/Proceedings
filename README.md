...
title/author/date/abstract, asks Claude for a ~500–1000 word thematic review (citing
every paper, with a verify/repair pass), turns the citation markers into hyperlinks,
and writes `literature-review.md` with a `review-meta` header recording the corpus hash
it was generated from. Other flags:
...
**Safety.** Generation never destroys the approved review: with no API key, or on an
API error, it logs a warning and leaves `literature-review.md` untouched. Hand-edits
