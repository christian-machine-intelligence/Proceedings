...
│   ├── generate-review.py       # Claude API: refresh literature-review.md (run locally)
│   └── serve.py                 # Local dev server (port 8787)
├── _site/                       # Generated output (gitignored)
│   ├── index.html               # Landing page (generated inline by build script)
│   ├── review.html              # Rendered literature review ("trailhead")
...
only renders it. This means you review and approve the exact text before it goes live.

**Refreshing it.** After adding or editing a paper, regenerate the review locally:

```bash
# needs ANTHROPIC_API_KEY in the environment or a gitignored ./.env
python3 scripts/generate-review.py            # regenerate if the corpus changed
git diff literature-review.md                 # review the change…
# …edit by hand if you like, then commit it
```
...
- `--check` — report whether the review is stale vs. the papers; **no API call**. The
  ...
  refreshing. Add `--strict` to exit non-zero instead (e.g. to gate CI).
- `--force` — regenerate even if the corpus is unchanged (re-runs are otherwise a
  no-op, so they never clobber your hand-edits).
- `--dry-run` — print the prompt only.
- `ICMI_REVIEW_MODEL` env var overrides the model (default `claude-opus-4-8`).

**File structure: a frozen preamble + a regenerated body.** `literature-review.md` has
two parts separated by a marker line:

```
<!-- review-meta: {...} -->          ← managed by the script (corpus hash)
<masthead image, intro, mission, the three-theme bullets>   ← YOUR hand-written preamble
<!-- review-body:start -->           ← boundary marker
## How might Christian representations be activated…         ← the three category sections
## How is one to measure and evaluate Christian virtue…         (regenerated by the script,
## How might Christian theology and doctrine inform…             every paper auto-categorized)
…closing line + footer…
```

Regeneration **preserves everything above `<!-- review-body:start -->` verbatim** and
rewrites only the three category sections and footer below it. So you can hand-curate the
intro, mission statement, and masthead freely; refreshing for a new paper won't touch
them. The three categories are fixed in `CATEGORIES` at the top of the script. If the
file has no `review-body:start` marker, the script refuses to regenerate (so it can't
overwrite an unstructured hand-edit) unless you pass `--force`.

**Safety.** Generation never destroys the approved review: with no API key, on an API
error, or when the marker is missing, it logs a warning and leaves `literature-review.md`
untouched. The body survives until you deliberately regenerate (a new paper or
`--force`), at which point only the body is rewritten — review the `git diff`.

**Rendering.** `build-site.sh` strips the `review-meta` and `review-body` marker lines
and renders the markdown to `_site/review.html` via `review-template.html`. The site
header's **Primer** nav link points to that page.

### Deployment

GitHub Actions (`.github/workflows/build-site.yml`) triggers on push to `main`:
1. Installs pandoc + texlive on Ubuntu
2. Runs `build-site.sh`, which renders the committed `literature-review.md` (no API
   call — it warns non-fatally if the review is stale relative to the papers)
3. Deploys `_site/` to GitHub Pages via `actions/deploy-pages@v4`

> No `ANTHROPIC_API_KEY` secret is needed for deploys — the key is only used locally
> when you run `generate-review.py` to refresh the review.
